  ...
  return deduplicatedModules.map((m: any, mIdx: number) => {
    ...
    const normalizedLessons: any[] = uniqueRawLessons.map((l: any, lIdx: number) => {
      ...
      return {
        ...
        revision: typeof l.revision === 'number' ? l.revision : (l.revision !== undefined && !isNaN(Number(l.revision)) ? Number(l.revision) : undefined),
        expectedRevision: typeof l.expectedRevision === 'number' ? l.expectedRevision : (typeof l.revision === 'number' ? l.revision : (l.revision !== undefined && !isNaN(Number(l.revision)) ? Number(l.revision) : undefined)),
