...
export class FirebaseFirestoreService {
  /**
   * Creates Firestore student document under `students` collection (with doc ID = user.uid)
   */
  public async createStudentDocument(data: StudentFirestoreDocument): Promise<void> {
    ...
    try {
      ...
      this.cacheStudentLocally(data);
      throw new Error(error?.message || 'Failed to save student details in database.');
    }
  }

  /**
   * Local storage fallback cache for offline development mode
   */
  private cacheStudentLocally(data: StudentFirestoreDocument): void {
    try {
      ...
      console.warn('Local storage fallback warning:', e);
    }
  }
}
