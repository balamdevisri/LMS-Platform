...
export const SESSION_TYPES = [
  { value: 'academic_lecture', label: 'Academic Course Class' },
  { value: 'technical_workshop', label: 'Technical Workshop' },
  { value: 'non_technical_session', label: 'Non-Technical Session' },
  { value: 'group_discussion', label: 'Group Discussion' },
  { value: 'panel_discussion', label: 'Panel Discussion' },
  { value: 'webinar', label: 'Webinar' },
  { value: 'guest_lecture', label: 'Guest Lecture' },
  { value: 'student_club', label: 'Student Club Activity' },
  ...
  { value: 'general_live_session', label: 'General Live Session' },
];
...
export const AdminCreateLiveClass: React.FC = () => {
  ...
  // Form State
  const [sessionCategory, setSessionCategory] = useState<'course' | 'general'>('course');
  const [sessionType, setSessionType] = useState<string>('academic_lecture');
  const [guestSpeakers, setGuestSpeakers] = useState<string>('');
  ...
  useEffect(() => {
    if (isEditing && id) {
      ...
      if (existing) {
        if ((existing as any).sessionCategory) {
          setSessionCategory((existing as any).sessionCategory);
        } else if (!existing.courseId || existing.courseId === 'course_default') {
          setSessionCategory('general');
        } else {
          setSessionCategory('course');
        }
        if ((existing as any).sessionType) setSessionType((existing as any).sessionType);
        if ((existing as any).topicCategory) setTopicCategory((existing as any).topicCategory);
        if ((existing as any).registrationRequired !== undefined) setRegistrationRequired(Boolean((existing as any).registrationRequired));
        if ((existing as any).guestSpeakers && Array.isArray((existing as any).guestSpeakers)) {
          setGuestSpeakers((existing as any).guestSpeakers.join(', '));
        }
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent, shouldPublish: boolean = false) => {
    ...
    if (sessionCategory === 'course' && !courseId) {
      toast.error('Please select a Course Program for this Course-Based Live Class.');
      return;
    }
    if (!instructorId) {
    ...
    const payload: any = {
      ...
      sessionCategory,
      sessionType,
      courseId: sessionCategory === 'course' ? courseId : undefined,
      courseName: sessionCategory === 'course' ? courseName : (topicCategory || 'General Live Session'),
      moduleId: sessionCategory === 'course' ? (moduleId || undefined) : undefined,
      moduleTitle: sessionCategory === 'course' ? (moduleTitle || undefined) : undefined,
      topicId: sessionCategory === 'course' ? (topicId || undefined) : undefined,
      lessonId: sessionCategory === 'course' ? (topicId || undefined) : undefined,
      lessonTitle: sessionCategory === 'course' ? (topicTitle || undefined) : undefined,
      guestSpeakers: guestSpeakers.split(',').map((s) => s.trim()).filter(Boolean),
      ...
      audienceTargeting: {
        type: targetAudience === 'enrolled_students' ? 'course' : targetAudience === 'selected_batch' ? 'batch' : targetAudience === 'selected_section' ? 'branch' : 'all',
        values: targetAudience === 'selected_batch' ? [targetBatch] : targetAudience === 'selected_section' ? [targetSection] : undefined,
      },
      permissions: {
        chatEnabled: interactionSettings.chat.enabled,
        studentMicEnabled: interactionSettings.studentMic.enabled,
        studentCamEnabled: interactionSettings.studentCamera.enabled,
        raiseHandEnabled: interactionSettings.raiseHand.enabled,
        pollsEnabled: interactionSettings.polls.enabled,
        quizzesEnabled: interactionSettings.quiz.enabled,
        hostScreenShareOnly: true,
      },
      interactionSettings: {
        ...interactionSettings,
        studentScreenShare: { enabled: false }, // Enforced host screen sharing only
      },
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 shadow-xs space-y-8">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-zinc-800">
            ...
            {/* CATEGORY & SESSION TYPE SELECTOR */}
            <div className="space-y-4 pb-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  onClick={() => {
                    setSessionCategory('course');
                    if (sessionType === 'technical_workshop' || sessionType === 'general_live_session') {
                      setSessionType('academic_lecture');
                    }
                    if (targetAudience === 'all') {
                      setTargetAudience('enrolled_students');
                    }
                  }}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-extrabold text-slate-900 dark:text-zinc-100">
                        Course-Based Live Class
                      </span>
                    </div>
                    {sessionCategory === 'course' && <CheckCircle2 className="w-4 h-4 text-blue-600" />}
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-zinc-400 mt-1.5 leading-relaxed">
                    Tied to course syllabus, modules, and enrolled students.
                  </p>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    setSessionCategory('general');
                    if (sessionType === 'academic_lecture') {
                      setSessionType('technical_workshop');
                    }
                    if (targetAudience === 'enrolled_students') {
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {sessionCategory === 'general' && <CheckCircle2 className="w-4 h-4 text-indigo-600" />}
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-zinc-400 mt-1.5 leading-relaxed">
                    Workshops, webinars, panels, guest lectures, student clubs, and mentorship.
                  </p>
                </button>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-extrabold text-slate-800 dark:text-zinc-200 uppercase tracking-wider block">
                  Live Session Type <span className="text-rose-500">*</span>
                </label>
                <select
                  value={sessionType}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {/* Course Selection — only required for Course-Based sessions */}
              {sessionCategory === 'course' ? (
                <div className="space-y-1.5">
                  <label className="text-xs font-extrabold text-slate-800 dark:text-zinc-200 uppercase tracking-wider block">
                ...
                /* Topic Category for General Live Session */
                <div className="space-y-1.5">
                  <label className="text-xs font-extrabold text-slate-800 dark:text-zinc-200 uppercase tracking-wider block">
                  ...
                  <input
                    ...
                    placeholder="e.g. AI & Machine Learning, Cloud Architecture, Career"
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-zinc-100 text-xs font-medium focus:outline-hidden focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              )}
              ...
              {sessionCategory === 'course' && topicOptions.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-xs font-extrabold text-slate-800 dark:text-zinc-200 uppercase tracking-wider block">
              ...
              {/* General Session Specific: Guest Speakers & Registration */}
              {sessionCategory === 'general' && (
                <>
                  <div className="space-y-1.5">
                    <label className="text-xs font-extrabold text-slate-800 dark:text-zinc-200 uppercase tracking-wider block">
                      Guest Speakers <span className="text-slate-400 font-medium normal-case">(optional, comma-separated)</span>
                    </label>
                    <input
                      type="text"
                      value={guestSpeakers}
                      onChange={(e) => setGuestSpeakers(e.target.value)}
                      placeholder="e.g. Dr. Alex Chen (Google), Sarah Connor (Meta)"
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-zinc-100 text-xs font-medium focus:outline-hidden focus:ring-2 focus:ring-blue-500"
                    />
                  </div>

                  <div className="space-y-1.5 flex flex-col justify-center">
                    <label className="text-xs font-extrabold text-slate-800 dark:text-zinc-200 uppercase tracking-wider block mb-1">
                      Event Registration
                    </label>
                    <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-xs font-semibold text-slate-800 dark:text-zinc-200 cursor-pointer hover:border-indigo-400 transition-colors">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span>Require Prior Registration for Entry</span>
                    </label>
                  </div>
                </>
              )}
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-zinc-800">
            ...
            <div className="mt-4 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200/80 dark:border-zinc-800">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                ...
                <div className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-100 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700/60 text-xs font-semibold text-slate-500 dark:text-zinc-400 cursor-not-allowed">
                  <ShieldCheck className="w-4 h-4 text-blue-500 shrink-0" />
                  <div className="flex flex-col">
                    <span>Host Screen Share Only</span>
                    <span className="text-[9px] text-slate-400 font-normal">Student screen share restricted</span>
                  </div>
                </div>
