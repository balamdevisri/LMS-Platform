...
const formatClassDateTime = (startTime?: string, createdAt?: string) => {
  const target = startTime || createdAt;
  if (!target) return { date: 'Today', time: 'Live Now' };
  const d = new Date(target);
  if (isNaN(d.getTime())) return { date: 'Today', time: 'Live Session' };
  return {
...
export const LiveClassroomDashboard: React.FC = () => {
  ...
  const filteredClasses = useMemo(() => {
    const now = new Date();
    if (filter === 'today') return classes.filter((c) => (isToday(c.startTime) || normalizeLiveClassStatus(c.status) === 'live') && normalizeLiveClassStatus(c.status) !== 'completed');
  ...
  const handleConfirmLiveAction = async () => {
    ...
    try {
      if (confirmActionType === 'enter') {
        ...
        const currentUid = (userProfile?.uid || '').trim();
        const currentEmail = (userProfile?.email || '').trim().toLowerCase();
        const currentName = (userProfile?.fullName || userProfile?.name || '').trim().toLowerCase();

        const instId = (confirmLiveClass.instructorId || '').trim();
        const createdBy = (confirmLiveClass.createdBy || '').trim();
        const instEmail = ((confirmLiveClass as any).instructorEmail || '').trim().toLowerCase();
        const instName = (confirmLiveClass.instructorName || '').trim().toLowerCase();
        ...
        if (!isLiveNow && isAssigned) {
          ...
          await liveClassService.startLiveClass(confirmLiveClass.id, userProfile?.uid, undefined, userMeta);
          toast.success(`🔴 "${confirmLiveClass.title}" is now LIVE!`);
        }
        ...
        toast.info('Session ended and removed from active schedule.');
      }
    } catch (err: any) {
        ...
        <div>
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800/80 text-blue-700 dark:text-cyan-300 text-xs font-bold uppercase tracking-wider">
              ...
              <span>{userProfile?.role === 'student' || !userProfile?.role ? 'STUDENT LIVE CLASSROOM PORTAL' : 'INSTRUCTOR LIVE CLASSROOM STUDIO'}</span>
            </div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800/80 text-emerald-700 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider">
              ...
              <span>REAL-TIME BROADCAST DATA</span>
            </div>
          </div>
          <h1 className="font-heading font-extrabold text-2xl sm:text-3xl text-slate-900 dark:text-white">
            {userProfile?.role === 'student' || !userProfile?.role ? 'Live Interactive Classrooms' : 'Instructor Live Dashboard'}
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium max-w-2xl leading-relaxed">
            {userProfile?.role === 'student' || !userProfile?.role
              ? 'Join live sessions conducted by expert mentors, participate in interactive coding labs, real-time polls, and ask questions.'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredClasses.map((c, idx) => {
            ...
            return (
              <div
                key={uniqueKey}
                className={`bg-white dark:bg-slate-900/90 rounded-3xl border transition-all flex flex-col justify-between overflow-hidden shadow-xs hover:shadow-2xl group ${
                  isLiveNow
                    ? 'border-rose-400 dark:border-rose-600 ring-2 ring-rose-500/20 shadow-rose-500/10'
              ...
              >
                ...
                <div className="relative h-48 bg-slate-950 overflow-hidden shrink-0">
                  <img
                    ...
                    alt={c.title || 'Live Classroom Session'}
                  ...
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2">
                    <span className="px-2.5 py-1 rounded-xl bg-slate-900/90 backdrop-blur-md text-sky-300 font-mono text-[10px] font-bold border border-slate-700/80 shadow-xs flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-sky-400" />
                      <span>{c.difficulty ? `${c.difficulty} Level` : 'Interactive Lab'}</span>
                    </span>

                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider shadow-sm flex items-center gap-1.5 ${
                        isLiveNow
                          ? 'bg-rose-600 text-white animate-pulse ring-2 ring-rose-400/40'
                          : c.status === 'Scheduled'
                  ...
                  <div className="absolute bottom-3 left-3 right-3">
                    <span className="inline-block px-2.5 py-1 rounded-lg bg-sky-950/80 backdrop-blur-md text-sky-300 font-bold text-[10px] uppercase tracking-wider border border-sky-400/30 shadow-xs">
                      {c.courseName || 'Advanced AI Systems Engineering'}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-3.5">
                    ...
                    <div className="space-y-1">
                      <h3 className="font-heading font-black text-base text-slate-900 dark:text-white leading-snug line-clamp-2 group-hover:text-sky-500 dark:group-hover:text-sky-400 transition-colors">
                        {c.title || 'Advanced AI Systems & Real-Time Engineering'}
                      </h3>
                      <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 truncate">
                        {c.moduleTitle ? `${c.moduleTitle}` : c.lessonTitle || 'Real-Time Interactive Live Engineering Stream'}
                      </p>
                    </div>

                    {/* Description */}
                    <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 leading-relaxed font-medium">
                      {c.description || 'Hands-on practical live stream with interactive doubt solving and real-time student collaboration.'}
                    </p>

                    {/* Dedicated Instructor Profile Spotlight */}
                    <div className="flex items-center gap-3 p-3 rounded-2xl bg-sky-50/50 dark:bg-slate-950/80 border border-sky-100 dark:border-slate-800 shadow-xs">
                      <div className="relative shrink-0">
                        {c.instructorAvatar ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-linear-to-tr from-sky-500 to-indigo-600 text-white font-black text-xs flex items-center justify-center border-2 border-sky-400 shadow-xs">
                            {instInitial}
                          </div>
                        )}
                    ...
                    {/* Schedule / Live Timing Box (No Invalid Date!) */}
                    <div
                      className={`rounded-2xl p-3 text-xs border ${
                        isLiveNow
                          ? 'bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-900/50 text-rose-700 dark:text-rose-300 shadow-xs'
                          : 'bg-sky-50/60 dark:bg-slate-950/80 border-sky-100 dark:border-slate-800 text-slate-700 dark:text-slate-300'
                      }`}
                    >
                      <div className="flex items-center justify-between font-semibold">
                        <div className="flex items-center gap-2">
                          {isLiveNow ? (
                            <>
                              ...
                              <span className="font-extrabold text-rose-600 dark:text-rose-400">Broadcasting Live Now</span>
                            </>
                          ) : (
                            <>
                        ...
                        <span className="font-mono text-[11px] font-bold text-slate-600 dark:text-slate-300">
                          {isLiveNow ? `${c.duration || 90} Min Session` : dt.time}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap text-[10px] font-bold">
                      <span className="px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 flex items-center gap-1">
                        ...
                        {c.maxParticipants || 100} Max Seats
                      </span>
                      <span className="px-2 py-0.5 rounded-lg bg-sky-50 dark:bg-sky-950/60 text-sky-700 dark:text-sky-300 border border-sky-200 dark:border-sky-800">
                        💬 Live Chat
                      </span>
                      ...
                      {c.notesUrl && (
                        <span className="px-2 py-0.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                          📄 Notes Attached
                        </span>
                      )}
                      {c.recordingUrl && (
                        <span className="px-2 py-0.5 rounded-lg bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                          🎥 Recording
                        </span>
                      )}
                      {c.isChatMuted && (
                        <span className="px-2 py-0.5 rounded-lg bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
                          🔇 Chat Muted
                        </span>
                      )}
                    </div>
                      ...
                      <>
                        <div className="flex items-center justify-between gap-2">
                          {(() => {
                            const isAssigned =
                              userProfile?.role === 'admin' ||
                              (userProfile?.uid && (c.instructorId === userProfile.uid || c.createdBy === userProfile.uid)) ||
                              (userProfile?.email && (c as any).instructorEmail && (c as any).instructorEmail.toLowerCase() === userProfile.email.toLowerCase()) ||
                              ((userProfile?.fullName || userProfile?.name) && c.instructorName && c.instructorName.toLowerCase().includes((userProfile?.fullName || userProfile?.name || '').toLowerCase())) ||
                              (['inst_kaizen', 'inst_default', 'instructor_lead', 'admin', ''].includes(c.instructorId) && (userProfile?.role === 'instructor' || userProfile?.role === 'mentor'));

                            if (isLiveNow) {
                              return isAssigned ? (
                                <button onClick={() => handleOpenExitConfirm(c)} className="flex-1 py-2.5 px-3 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md">
                                  <StopCircle className="w-4 h-4" /><span>End Session</span>
                                </button>
                              ) : (
                                <button onClick={() => handleOpenEnterConfirm(c)} className="flex-1 py-2.5 px-3 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md animate-pulse">
                                  <Radio className="w-4 h-4" /><span>Join Live</span>
                                </button>
                              );
                            }

                            return isAssigned ? (
                              <button onClick={() => handleOpenEnterConfirm(c)} className="flex-1 py-2.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow-md">
                                <Play className="w-4 h-4" /><span>Start Live</span>
                              </button>
                            ) : (
                              <div className="flex-1 py-2 px-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-bold text-center truncate">
                                Lead: {c.instructorName || 'Mentor'}
                              </div>
                            );
                          })()}
                          <button onClick={() => handleOpenEnterConfirm(c)} className="py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-md">
