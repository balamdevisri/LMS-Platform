...
export interface CleanerOptions {
  languageHint?: 'bash' | 'python' | 'javascript' | 'typescript' | 'json' | 'text';
}
 ...
 */
export function healHyphenatedWords(text: string): string {
  if (!text) return '';
 ...
 */
export function healOrphanLinebreaks(text: string): string {
  if (!text) return '';
  ...
  while (i < lines.length) {
    ...
    if (
      ...
      while (j < lines.length) {
        ...
        if (lookAheadIdx >= lines.length) break;
 ...
 */
export function wrapAsciiDiagrams(text: string): string {
  if (!text) return '';
 ...
 */
export function cleanCourseContent(rawContent: string, _options: CleanerOptions = {}): string {
